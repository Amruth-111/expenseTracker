...
window.addEventListener("DOMContentLoaded",async()=>{
    try{
        const allInfo=await axios.get("http://localhost:2500/get-expense")
        console.log(allInfo)
...
async function deleteExpense(key){  
    await axios.delete(`http://localhost:2500/delete-expense/${key}`)  
    const child=document.getElementById(key)
    parentNode.removeChild(child)
}

//Edit functionality
async function editExpense(key,amt,desc,cat){
    try{
        amount.value=amt
...
async function addExpense(e){
    try{
       ...
       const allInfo=await axios.post("http://localhost:2500/add-expense",obj)
       console.log(allInfo)
           showOnScreen(allInfo.data.newUser)
